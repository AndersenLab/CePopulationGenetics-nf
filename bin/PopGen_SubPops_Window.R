...
window_pos <- data.frame(PopGenome::get.neutrality(GENOME_OBJECT)[[1]]) %>%
...
for(popnumber in 1:length(POPGENOME_POPS)){
  ...
  n_df <- data.frame(PopGenome::get.neutrality(GENOME_OBJECT)[[popnumber]]) %>%
    dplyr::bind_cols(window_pos,.)
  
  d_df <- data.frame(PopGenome::get.diversity(GENOME_OBJECT)[[popnumber]]) %>%
    dplyr::bind_cols(window_pos,.)
  
  neutrality_ls[[popnumber]] <- dplyr::left_join(n_df,d_df, by = c("CHROM", "startWindow", "endWindow")) %>%
    dplyr::ungroup() %>%
    dplyr::mutate(Population = popname,
                  theta_Watterson = GENOME_OBJECT@theta_Watterson[,popnumber]) %>%
    tidyr::gather(statistic, value, -(CHROM:endWindow), -Population) %>%
